...
def run_ndt (addr):
   ...
   test_output = subprocess.Popen(["ssh", addr, ndt_cmd],stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
   log_text = test_output.communicate()
   thingy = open("beta.log", 'a')
   thingy.write(time.strftime("\n-------\n%x,%H:%M:%S\n" + ndt_server + "\n"))
   for line in log_text[0].split('\n'):
      print line
      thingy.write(line + "\n")
   thingy.close()
   #This will only work if the SSH infrastucture on the governor and the probes has been set up correctly
...
while True:
   ...
   #Assemble the list of today's period boundaries by mashing together today's date and the generic hour:minute entries from the config file
   period_boundaries = [time.strptime(time.strftime("%x", now) + "," + t, "%x,%H%M") for t in config.get("Timing", "boundaries").split()]
   #Identify the next period boundary
...
#            print str(last_run) + ", " + str(next_period)
   ...
   #TODO: This logic will break at the day boundary at the end of a leapyear, when %j will be incremented to 367, which is out of the range "time" accepts 
      ...
      #TODO:Explicitly sort the boundary list before blindly grabbing the first element?
...
#load buckets
#assume that current bucket has not been run yet
#schedule using random(now, bucket boundary)
#sleep
#Wake up, run test, finish loop.  
